...
public class StatusPanel extends View
{
    ...
    public void update(int xArg, int yArg, int widthArg, int heightArg, float scaleArg, int switchImageCnt)
    {
        x = xArg;
        y = yArg;
        w = widthArg;
        h = heightArg;
        imageScale = scaleArg;
        switchImageCount = switchImageCnt;
        invalidate();
    }
    ...
    protected void onDraw(Canvas canvas)
    {
        ...
        canvas.drawText(String.format(Locale.CANADA, "double-tap = %d", switchImageCount), margin + 1 * fieldWidth, 3 *
                (textSize + textSize / 4), p);
    }
}
